...
def main():
    ...
    if uploaded_file:
        ...
        # Create a table to store the relative performance
        performance_table = pd.DataFrame(index=indexes.keys())
        ...
        for score, group in grouped_stocks:
            ...
            # Calculate the difference between the group's average performance and each index
            differences = {}
            for name, ticker in indexes.items():
